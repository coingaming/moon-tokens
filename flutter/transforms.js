...
module.exports = {
  ...
  "gradient/flutter": {
    ...
    transformer: ({ value: { gradientType, stops, rotation } }) => {
      ...
      // Positions
      var stopValues = stops.map((x) => n(x["position"]));
      result += `stops: [${stopValues.join(", ")}],`;

      // Colors
      var colorValues = stops.map((x) => generateColor(x["color"]));
  ...
  "radius/flutter": {
    ...
    transformer: function ({
      value: { topRight, topLeft, bottomLeft, bottomRight },
    }) {
      if ([topRight, bottomLeft, bottomRight].every((v) => v === topLeft)) {
        return `BorderRadius.all(Radius.circular(${n(topLeft)}))`;
      }
      return `BorderRadius.only(topLeft: Radius.circular(${n(
        topLeft
      )}), topRight: Radius.circular(${n(
        topRight
      )}),bottomRight: Radius.circular(${n(
        bottomRight
      )}),bottomLeft: Radius.circular(${n(bottomLeft)}),)`;
    },
  },
  "spacing/flutter": {
    type: "value",
    ...
    transformer: function ({ value: { left, top, right, bottom } }) {
      ...
      if (left === right && top === bottom) {
        return `EdgeInsets.symmetric(vertical: ${n(top)}, horizontal: ${n(
          left
        )})`;
      }

      return `EdgeInsets.only(left: ${n(left)}, right: ${n(right)}, top: ${n(
        top
      )}, bottom: ${n(bottom)})`;
    },
  },
